export const Courses = () => (
    <section className="courses">
      <h1 className="heading"> Our Skill Courses </h1>
      <div className="box-container">
        <div className="box">
          <div className="image">
            <img src="/public/course-2-1.jpg" alt="" />
            <h3>Coding</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="coding.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
        <div className="box">
          <div className="image">
            <img src="/public/course-2-2.jpg" alt="" />
            <h3>Electro Machanical Production</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="emp.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
        <div className="box">
          <div className="image">
            <img src="/public/finance2.webp" alt="" />
            <h3>Finance &amp; Accounting</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="Finance.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
        <div className="box">
          <div className="image">
            <img src="/public/course-2-7.jpg" alt="" />
            <h3>Digital media &amp; designing</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="dmd.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
        <div className="box">
          <div className="image">
            <img src="/public/fashion1.jpg" alt="" />
            <h3>Fashion designing</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="fashion.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
        <div className="box">
          <div className="image">
            <img src="/public/robot skill.jpg" alt="" />
            <h3>Robotices</h3>
          </div>
          <div className="content">
            <h3>choose what's best for you!</h3>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Eaque,
              odit!
            </p>
            <a href="robotics.html" className="btn">
              read more
            </a>
            <div className="icons">
              <span>
                {" "}
                <i className="fas fa-clock" /> 2 hours{" "}
              </span>
            </div>
          </div>
        </div>
      </div>
      {/*<div class="load-more"> <div class="btn">load more</div> </div>*/}
    </section>
  )
  